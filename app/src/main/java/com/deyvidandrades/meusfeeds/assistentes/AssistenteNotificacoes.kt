...
object AssistenteNotificacoes {
    ...
    fun notificacaoDiaria(context: Context) {
        ...
        if (Persistencia.horarioNotificacao != -1) {
            val listaGrupos = Persistencia.getFeedGroups()
            listaGrupos.shuffle()

            kotlinx.coroutines.runBlocking {
                val result = RequestManager.fazerRequisicao(URL(listaGrupos[0].url))

                RssParser.getArrayArtigos(listaGrupos[0], result, 1) {
                    if (it.isNotEmpty()) {
                        val artigo = it[0]
                        ...
                        val builder = NotificationCompat.Builder(context, CHANNEL_ID)
                            ...
                            .setContentTitle(artigo.titulo)
                            .setContentText(artigo.descricao)
